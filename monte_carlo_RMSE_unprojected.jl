...
@everywhere function run_point(snr, x_star, u_star, T, grid, k_0, k_1, ϵ_values;
	...
	[
		 monte_carlo_RMSE_unprojected(x_star, u_star, snr, T, grid, k_0, k_1;
			ϵ = ϵv, trials = trials,
...
snr_range = range(24, 30, snr_samples)
...
unprojected_rmse_map = reduce(vcat, [reshape(r, 1, :) for r in results_rows])

# Save
writedlm("results/tsp_monte_carlo_unprojected_rmse_map.txt",
	(ϵ_range_unprojected, snr_range, unprojected_rmse_map))