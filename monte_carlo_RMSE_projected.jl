...
@everywhere function run_row_projected(snr, x_star, θ_star, k_0, T, grid,
	...
	[
		monte_carlo_RMSE_projected(x_star, θ_star, snr, ϵ, k_0, T, grid;
...
snr_range = range(15, 25, snr_samples)
...
# Stack into matrix
projected_rmse_map = reduce(vcat, [reshape(r, 1, :) for r in results_rows])

# Save
writedlm("results/tsp_monte_carlo_projected_RMSE_map.txt",
	(ϵ_range_unprojected, snr_range, projected_rmse_map))
